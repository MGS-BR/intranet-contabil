...
for /f "delims=" %%i in ('python -c "import json; print(json.load(open(r'%BASE%\config.json'))['LOG_BACKUP'])"') do set LOG_BACKUP=%%i

robocopy "%PASTA%" "%BACKUP%" /E /R:1 /W:2 /MT:8 /FFT /XJ /XA:SH /XF desktop.ini thumbs.db *.tmp *.temp /TEE /LOG:"%LOG_BACKUP%"
